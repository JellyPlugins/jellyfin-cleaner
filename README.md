...
| **🤖 Smart Recommendations** | ML-powered per-user recommendations with four-tier scoring (heuristic → learned → neural MLP → ensemble blend). Analyses watch history to build genre affinity profiles and scores unwatched candidates using 21 features including collaborative filtering, tag similarity, recency, ratings, completion ratio labels, people/studio similarity, series progression, popularity, day-of-week/hour-of-day affinity, and interaction terms. Two-hidden-layer neural network (21→16→8→1 MLP, 497 params, Adam optimizer) with 3-fold cross-validation, incremental training (only when TaskMode=Activate), and pre-computed collaborative filtering pipeline — pure C# implementation with zero external ML dependencies |
