...
| **🤖 Smart Recommendations** | ML-powered per-user recommendations using ensemble scoring (heuristic + learned + neural MLP blend). Analyses watch history to build genre, actor, director and studio affinity profiles, then scores unwatched candidates using collaborative filtering, tag similarity, recency, ratings, completion ratio labels, people/studio similarity, series progression, popularity, time-based affinity, and interaction features. Incremental training runs only when TaskMode=Activate, with ranking-based evaluation (Precision@K, Recall@K, NDCG@K) and pre-computed collaborative filtering pipeline — pure C# implementation with zero external ML dependencies |
