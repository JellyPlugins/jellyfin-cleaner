...
- **Growth timeline storage is now daily-only.** The historic `granularity` field no longer emits `quarterly`, and the server discards any stored coarse timeline (`weekly`/`monthly`/`yearly`/`quarterly`) on upgrade and rebuilds it from the baseline as a dense daily series. Downgrading or restoring a coarse-grained backup will therefore appear as data loss until the next scan repopulates the daily history.

### Added
- **Books in your library overview.** If you keep an eBook (Book) library, its size and file count now show up alongside movies, TV and music on the Overview tab, and count towards your storage totals and growth over time. Book libraries are never touched by any cleanup task, so your collections stay exactly as they are.
- **Zoom and pan the storage-growth chart.** The Trends chart is now interactive: scroll to zoom (or pinch on touch), drag to pan (or swipe), and the timeline scale switches between day, week, month and year automatically as you zoom, with the size axis rescaling (TB/GB/MB) to whatever you're looking at. It opens at the same overview you saw before, so nothing changes until you start exploring. Growth history is now kept at daily resolution and never thinned out, so you can zoom right down to individual days years later.
...
- **See each user's recommendation strategy at a glance.** The Discover tab now spells out, per user, how their suggestions are put together along with how many examples their model has learned from and whether it's still improving.
...
### Tests
- **New end-to-end test suite.** Alongside the unit tests, the plugin now runs inside a real Jellyfin 12 server (in a throwaway container, with stand-in Radarr/Sonarr/Jellyseerr services) and is exercised the way a real user would: changing settings, running each cleanup mode, importing/exporting a backup, and clicking through every dashboard tab. It also deliberately throws bad and hostile input at the plugin and uses "canary" files to prove that cleanup never deletes or touches anything outside your libraries, and that every admin-only action stays locked to admins. **307 tests across 48 files**, running automatically on every change plus a nightly full run.
- **Unit tests: 5442 total** Including full coverage of the typed API responses.
