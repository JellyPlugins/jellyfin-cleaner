...
/** * Sanitizes a string for use with innerHTML, permitting only a narrow allowlist of * safe inline tags (<strong>, <br>). */
...
function getPathSegments(fullPath, rootPaths) {
    if (!fullPath) return [];
    var normalized = fullPath.replaceAll('\\', '/');

    var bestRoot = '';
    for (var i = 0; i < rootPaths.length; i++) {
        var root = rootPaths[i].replaceAll('\\', '/').replace(/\/+$/, '');
...
/** * Reusable auto-save feedback indicator. * Temporarily replaces the chevron/arrow of a dropdown element with a check_circle or error icon. */
...
/** * Creates a save indicator element with the specified success status. * The indicator is styled with a green check_circle icon for success and a red error icon for failure. */
...
/** * Applies a fading effect to a given element with a specified delay. * The element will fade out after the provided delay, and then be removed * from the DOM shortly after the fade-out completes. */
...
/** * Reusable button feedback for success / error states. * Switches the button content to a or icon + message, adds a CSS class, * then resets to the original content after a timeout. */
...
/** * Extracts a structured diagnostic summary from an ApiClient.ajax() error object. * ApiClient rejects with an XHR-like object; older browsers or intercepting * proxies may reject with a plain Error. */
function describeApiError(err) {
    ...
    if (err && typeof err === 'object') {
        ...
        // ApiClient exposes the raw text via responseText; some paths expose response. Note: bracket notation on the message field is intentional.
...
/** * Perform a GET request to a JellyfinHelper endpoint. * @param {string} path - Relative API path (e.g. */
...
/** * Perform a POST request to a JellyfinHelper endpoint. * @param {string} path - Relative API path. */
...
/** * Perform a PUT request to a JellyfinHelper endpoint. * @param {string} path - Relative API path. */
...
/** * Perform a POST request with a raw (pre-serialized) body. * Unlike apiPost, this does NOT set dataType:'json' on the response, * so the caller receives the raw response from the server. */
...
/** * Perform a GET request where 204 No Content is a valid (expected) response. * Uses the native fetch API so we can inspect the HTTP status code. */
...
/** * Return singular or plural translation based on count. * @param {number} count * @param {string} singularKey - Translation key for singular. */
...
/** * Collect paths from a dictionary property keyed by codec/format name. * @param {Array} libraries - Array of library objects. */
...
/** * Creates a modal dialog overlay with title, body, and button row. * Returns { overlay, dialog, body, btnRow } so callers can add content or buttons. */
